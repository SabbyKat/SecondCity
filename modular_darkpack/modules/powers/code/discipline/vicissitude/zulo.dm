...
And these rows basically allow the carbon mob to override the original base tg human sprite in favor of, for example, the tzizulo1 sprite, correct?
...
*/
#define SPECIES_ZULO_FORM "zulo" // Sabby: Blood Form seems to do this species define for the carbon slime bloodform. Replicated here for consistency.
...
/datum/species/tzimisce_zulo_form
	...
	inherent_traits = list(
		...
		TRAIT_MASQUERADE_VIOLATING_FACE, // Sabby: pretty self-explanatory I feel
		...
		TRAIT_STAKE_IMMUNE, // Sabby: how exactly do you even stake a huge, alien flesh warform, after all?
	...
	no_equip_flags = ITEM_SLOT_MASK | ITEM_SLOT_OCLOTHING | ITEM_SLOT_GLOVES | ITEM_SLOT_FEET | ITEM_SLOT_ICLOTHING | ITEM_SLOT_SUITSTORE // Sabby: taken from oldcode zulo. Drops bunches of stuff, but should keep backpack, eyes, belt, head, neck, etc. Extremely important to keep backpack because of the dirt bag Tzim clan bane.
...
/* Below borrowed from shifter/dire. Zulo-specific bodyparts (limb behavior, damage, etc.) are defined in zulo_organs.dm. */
...
/* Do we want to set specific pixel offsets? Check oldcode. Below values are placeholders */
	...
	/// Sabby: note - Zulo should set social stats to 0. This is handled in code below on species gain, works differently from this list.
...
/datum/species/tzimisce_zulo_form/on_species_gain(mob/living/carbon/human/human_who_gained_species, datum/species/old_species, pref_load, regenerate_icons)
	...
	It follows the on_gain and on_loss update pattern used in lines and 174-175 and 187-188 of fera_species.dm */
	...
	old_manipulation = human_who_gained_species.st_get_stat(STAT_MANIPULATION) //Sabby: grabs and feeds into old_manipulation var the user's manipulation stat
	old_charisma = human_who_gained_species.st_get_stat(STAT_CHARISMA) //Sabby: grabs and feeds into old_charisma var the user's charisma stat
	...
	human_who_gained_species.remove_overlay(BODY_ADJ_LAYER) // Sabby: I tested this from werewolf code, it seems to do what I need in terms of removing Tzimisce beast marks in transformation.
...
going to take the buff amounts from a LIST type created in form_bonus_stats. And that list in there is set as an alist with key = value pattern?
...
Then it sets up clear_buffs which just *also* runs through the form_bonus_stats list and reverses them.
*/
